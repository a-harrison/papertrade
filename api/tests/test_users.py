...
@pytest.fixture()
async def setup_async_client(event_loop):
...
#
# @pytest.fixture()
# async def populate_gettable_user(create_db_connection):
#     test_db = create_db_connection
#     result = await test_db.users.find_one_and_update(
#     {
#         "_id" : "test_gettable_user"
#     },
#     {
#         "$set" : {
#             "username" : "test",
...
@pytest.mark.asyncio
async def test_get_user(setup_async_client, event_loop):
    ac = setup_async_client
    response = await ac.get(
        '/users/test_gettable_user'
    )
